...
import { DASHBOARDS, MONITOR_DASHBOARDS, MONITOR_TABS, SOLAR_EXTERNAL_URL } from '../config';
...
function MainArea({ activePage }) {
  ...
  const renderSolarLinkOut = () => (
    <div className="solar-linkout">
      <svg className="solar-linkout-icon" width="56" height="56" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="solar-linkout-title">{t.monitorSolar.title}</div>
      <div className="solar-linkout-desc">{t.monitorSolar.desc}</div>
      <a
        className="solar-linkout-btn"
        href={SOLAR_EXTERNAL_URL}
        target="_blank"
        rel="noopener noreferrer"
      >
        {t.monitorSolar.button} ↗
      </a>
    </div>
  );
  ...
  if (activePage === 'monitor' && monitorTab === 'solar') {
    return (
      <div className="main-area">
        {renderMonitorTabs()}
        {renderSolarLinkOut()}
